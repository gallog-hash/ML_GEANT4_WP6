...
class VAEOptimizer(BaseVAEPipeline):
    ...
    def create_study(self) -> optuna.Study:
        ...
        default_study_path = f"sqlite:///{(self.config.optuna_io_dir / db_name).resolve()}"
        ...
        if self.config.study_path and self.config.use_timestamp_output_dir:
            ...
            self.logger.info(
                f"Updated storage path to: {storage_path}"
            )
    ...
    def get_network_hyperparams(self, trial):
        ...
        log_params_dict("Network Architecture", {
    ...
    def get_exit_activation(self, trial):
        ...
        log_params_dict("Output Activation", {
        ...
        trial.set_user_attr("skip_norm_in_final", 
                            self.config.default_skip_norm_in_final)
    ...
    def get_loss_hyperparams(self, trial, loss_type):
        ...
        neg_penalty_weight = trial.suggest_float(
            "neg_penalty_weight", 0.1, 5.0, log=True
        ) if use_neg_penalty else 0.0
        ...
        loss_hyperparams = {
            ...
            "proc_dim": self.data_components['n_features'] - len(
                self.config.identity_features
            ),
        }
        ...
        if loss_type == "standard":
            loss_hyperparams["beta"] = trial.suggest_float(
                "beta", 0.1, 5.0, log=True
            )
    ...
    def train_vae_model(self, model, train_loader, val_loader, trial, 
                        val_loss_threshold):
        history = model.fit(
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        model = AutoEncoder(
            architecture_params=net_params, 
            device=self.device
        ).to(self.device)
        ...
        history = self.train_vae_model(model, train_loader, val_loader, trial,
            self.config.val_loss_threshold)
        
        # Use the final validation loss as the objective
        final_val_loss = (history['val_loss'][-1] if history['val_loss'] 
                        else float('inf'))
    ...
    def save_hparams_config(
        self, 
        trial, 
        save_path="best_hyperparameters_config.json"
    ):
...
def main(config_path: Optional[Union[str,Path]] = None):
    ...
    optimizer.save_history(
        path=config.optuna_io_dir / config.history_filename
    )
