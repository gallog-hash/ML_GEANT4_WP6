...
class VAEOptimizationAnalyzer(BaseVAEPipeline):
    def __init__(self, config: OptimizationAnalysisConfig):
        ...
        self.study = load_optuna_study(
            ...
            storage=self.config.study_path
        )
    ...
    def _create_capped_study(self, cap: float) -> optuna.Study:
        ...
        for trial in self.study.get_trials(deepcopy=True, states=[TrialState.COMPLETE]):
            if trial.value is not None:
                ...
                new_study.add_trial(optuna.trial.FrozenTrial(
                    number=trial.number,
        ...
        try:
            with warnings.catch_warnings():
                ...
                if fig1 and fig1.axes:
                    ...
                    display_plot(fig1)
                    save_figure(
                        fig1, self.config.output_dir, "optimization_history"
                    )
                else:
                    self.logger.warning(
                ...
                if fig2:
                    ...
                    self.logger.warning(
                        "plot_param_importances returned no figure."
                    )
    ...
    def plot_parameters_slice(self, batch_size=3):
        self.logger.info("Plotting slice plots of parameters in batches of 3 by "
                        "importance...")
        
        importances = optuna.importance.get_param_importances(self.study)
        sorted_params = [k for k, _ in sorted(
...
def main(config_path: Optional[Union[str, Path]] = None):
    ...
    if config_path is None:
        parser = argparse.ArgumentParser(
            description="VAE Optimization Analysis Script")
        parser.add_argument(
            "--config_path",
            type=str,
